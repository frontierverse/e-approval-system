    ...
    <>
      ...
      <section className="grid gap-6 xl:grid-cols-[minmax(0,1fr)_22rem]">
        <article className="rounded-md border border-[#d9dee7] bg-white p-5">
          ...
          <div className="mt-5 whitespace-pre-wrap text-sm leading-7 text-[#394150]">
            {resource.summary}
          </div>
        </article>

        <aside className="self-start rounded-md border border-[#d9dee7] bg-white p-5">
          ...
          {resource.attachments.length > 0 ? (
            <ul className="mt-4 divide-y divide-[#eef1f5] rounded-md border border-[#eef1f5]">
              {resource.attachments.map((attachment) => (
                <li key={attachment.id} className="px-3 py-2">
                  <AttachmentFileRow
